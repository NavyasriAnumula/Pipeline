...
# ============================================================================
CATEGORY = "TECHNOLOGIES"  # Options: "TOOLS", "TECHNOLOGIES", "COMPLIANCES"
...
def find_duplicate_groups_in_new_items(new_items, threshold=85):
    ...
    # Step 1: Find ALL pairwise similarities and union them
    match_count = 0
    for i in range(n):
        for j in range(i + 1, n):
            similarity = calculate_similarity(new_items[i]['item'], new_items[j]['item'])

            if similarity >= threshold:
                union(i, j)
                match_count += 1
                print(
                    f"   🔗 Matched: '{new_items[i]['item']}' <-> '{new_items[j]['item']}' (similarity: {similarity}%)")

    # Step 2: Group items by their root parent
    groups_dict = {}
    for i in range(n):
        root = find(i)
    ...
    # Step 3: Convert to required format with similarities
    groups = []
    for indices in groups_dict.values():
        group = []

        # For each item in the group, calculate its max similarity with any other item in group
        for i in indices:
            if len(indices) == 1:
                # Singleton - similarity is 100% with itself
                group.append((new_items[i], 100))
            else:
                # Calculate max similarity with other items in the group
                max_sim = 100  # At least 100% with itself
                for j in indices:
                    if i != j:
    ...
    print(f"      - Total pairwise matches found: {match_count}")

    # Print details of multi-item groups
    if duplicate_groups:
...
# ============================================================================
def  merge_and_classify_tech_pipeline():
...
#     merge_and_classify_tech_pipeline()